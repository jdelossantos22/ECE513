...
router.post('/readAll', function(req,res,next){
    ...
    //, $lt: Date(tomorrow)}
    //$gte: new Date(today.getFullYear(),today.getMonth(),today.getDate(),0,0,0).toISOString(), $lt: new Date(tomorrow.getFullYear(),tomorrow.getMonth(),tomorrow.getDate(),0,0,0).toISOString()
    Temperature.find({postDate:{$gte: today, $lte:tomorrow}, deviceId:req.body.id, userEmail:req.body.email}).sort({postDate:1}).exec(function(err, docs){
        if(err){
