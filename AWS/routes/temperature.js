...
router.post('/readAll', function(req,res,next){
    ...
    //console.log(req.body.email)
    //, $lt: Date(tomorrow)}
    Temperature.find({postDate:{$gte: new ISODate(today), $lt: new ISODate(tomorrow)}, deviceId:req.body.id, userEmail:req.body.email}).sort({postDate:1}).exec(function(err, docs){
        if(err){
