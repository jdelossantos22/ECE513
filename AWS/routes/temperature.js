...
router.post('/readAll', function(req,res,next){
    ...
    //console.log(req.body.email)
    //, $lt: Date(tomorrow)}
    Temperature.find({postDate:{$gte: Date(today).toISOString(), $lt: Date(tomorrow).toISOString()}, deviceId:req.body.id, userEmail:req.body.email}).sort({postDate:1}).exec(function(err, docs){
        if(err){
